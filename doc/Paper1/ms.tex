...
$p(\data|\params,\info)$
describes prior knowledge about the data we expect to observe, as a function of
$\params$ (Caticha). When $\data$ is fixed $p(\data|\params,\info)$ is a function of
...
distribution, and also the set of parameters (including nuisance parameters) to
be inferred.
...
\hline
$\tau_2$	&	Time delay relative to image 1	& Cauchy()$T(,)$\\
$\{m_1, m_2\}$	&	Mean magnitude for each image	& Uniform()\\
$L$		&	Length scale of QSO variations	& LogUniform()\\
$\{\ell_1, \ell_2\}$ &	Length scale of image-specific variations & \\
\hline
