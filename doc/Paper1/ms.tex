...
For inferring unknown parameters $\params$ from data $\data$,
Bayes' rule gives the posterior distribution
\begin{eqnarray}
p(\params | \data) \propto p(\params)p(\data|\params)
\end{eqnarray}
The prior distribution $p(\params)$ describes prior knowledge or uncertainty
...
$\params$ (Caticha). When $\data$ is fixed $p(\data|\params)$ is a function of
